...
powershell -C "$oldWallpaper = (Get-ItemProperty -Path 'HKCU:\Control Panel\Desktop' -Name WallPaper).WallPaper; if($oldWallpaper -and (Test-Path $oldWallpaper)) { $backupFile = Join-Path %APPDATA% 'info.txt'; \"%APPDATA%`n$oldWallpaper\" | Out-File -Encoding ASCII $backupFile }"
